...
__device__ __forceinline__ uint32_t calc_len_for_mul_small(uint32_t L_a, uint32_t L_c) {
    return min(L_c, L_a + 1u);
}

__device__ __forceinline__ void mul_word_parts(uint32_t a, uint32_t b, uint32_t & lo, uint32_t & hi) {
...
__global__ void batch_mul_small_naive_kernel(
    const uint32_t * A,
    uint32_t B,
    uint32_t * C,
    uint32_t N,
    ...
    const uint32_t calc_len = calc_len_for_mul_small(L_a, L_c);
    ...
    for (uint32_t idx0 = blockIdx.x * blockDim.x; idx0 < N; idx0 += gridDim.x * blockDim.x) {
        ...
        for (uint32_t i = 0; i < calc_len; ++i) {
            ...
            uint32_t hi = 0u;
            if (i < L_a) {
                mul_word_parts(a_row[i], B, lo, hi);
            }
...
__global__ void batch_mul_small_warp_kernel(
    const uint32_t * A,
    uint32_t B,
    uint32_t * C,
    uint32_t N,
    ...
    for (uint32_t idx = warp_global; idx < N; idx += warp_stride) {
        ...
        if (lane < calc_len) {
            ...
            uint32_t hi = 0u;
            if (lane < L_a) {
                mul_word_parts(a_row[lane], B, lo, hi);
            }
            const uint32_t addend = (lane > 0u && lane - 1u < L_a) ? ((uint32_t)(((uint64_t)a_row[lane - 1u] * (uint64_t)B) >> 32)) : 0u;
            word = lo + addend;
            carry_state = (word < lo) ? 2u : (word == 0xffffffffu ? 1u : 0u);
        }
        ...
        if (lane < L_c) {
            c_row[lane] = (lane < calc_len) ? (word + (carry_in >> 1)) : 0u;
        }
    }
}

__global__ void batch_mul_small_single_block_kernel(
    const uint32_t * A,
    uint32_t B,
    uint32_t * C,
    uint32_t N,
...
__global__ void batch_mul_small_reduce_blocks_kernel(
    const uint32_t * A,
    uint32_t B,
    uint32_t * C,
...
__global__ void batch_mul_small_combine_blocks_kernel(ushort2 * block_carry_summary, uint32_t N, uint32_t L_c) {
    __shared__ ushort2 carry_info[32];
...
__global__ void batch_mul_small_apply_blocks_kernel(
    uint32_t * C,
...
void batch_mul_small(
    ...
    if (L_c < 32u) {
        if (L_c <= 8u) {
            ...
            batch_mul_small_naive_kernel<32><<<num_blocks, threads_per_block>>>(
                A, B, C, N, L_a, L_c, stride_A, stride_C
            );
        } else {
            const uint32_t warps_per_block = 8u;
            const uint32_t num_blocks = std::min<uint32_t>((N + warps_per_block - 1u) / warps_per_block, 65536u);
            batch_mul_small_warp_kernel<<<num_blocks, dim3(32u, warps_per_block, 1u)>>>(
                A, B, C, N, L_a, L_c, stride_A, stride_C
            );
        }
        return;
    ...
    if (L_c <= kChunkSize || N >= 85u || workspace == nullptr) {
        ...
        batch_mul_small_single_block_kernel<<<num_blocks, dim3(32u, warps_y, 1u)>>>(
            A, B, C, N, L_a, L_c, stride_A, stride_C
        );
    } else {
        ...
        batch_mul_small_reduce_blocks_kernel<<<dim3(num_blocks_x, num_blocks_y, 1u), dim3(32u, 32u, 1u)>>>(
            A, B, C, block_carry_summary, N, L_a, L_c, stride_A, stride_C
        );
        batch_mul_small_combine_blocks_kernel<<<std::min<uint32_t>(N, 65536u), dim3(32u, 32u, 1u)>>>(
            block_carry_summary, N, L_c
        );
        batch_mul_small_apply_blocks_kernel<<<dim3(num_blocks_x, num_blocks_y, 1u), dim3(32u, 32u, 1u)>>>(
