...
void compute_reference_sub_small(
...
void print_limb_window(
    ...
    compute_reference_sub_small(a_row, B, expected.data(), L_a, L_c);
...
bool test_configuration(
    ...
    const size_t workspace_size = batch_sub_small_workspace_size(N, L_a, L_c);
    ...
    batch_sub_small(d_A, B, d_C, d_workspace, N, L_a, L_c, stride_A, stride_C);
...
bool test_in_place_configuration(
    ...
    uint32_t * d_workspace = nullptr;
    const size_t size_A = h_A.size() * sizeof(uint32_t);
    const size_t workspace_size = batch_sub_small_workspace_size(N, L, L);
    ...
    CUDA_CHECK(cudaMemcpy(d_A, h_A.data(), size_A, cudaMemcpyHostToDevice));
    batch_sub_small(d_A, B, d_A, d_workspace, N, L, L, L, L);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    CUDA_CHECK(cudaMemcpy(h_A.data(), d_A, size_A, cudaMemcpyDeviceToHost));
...
bool test_long_borrow_chain_case(
    uint32_t N,
    uint32_t L_a,
    uint32_t L_c,
    uint32_t stride_A,
    ...
    if (verbose) {
        printf("Testing long borrow chain %s case: N=%u, L_a=%u, L_c=%u, B=%08x...\n",
...
bool verify_sampled_results_with_gmp(
    ...
    for (size_t s = 0; s < h_A.size() / stride_A; ++s) {
        mpz_t a, b, diff, expected, actual;
        mpz_inits(a, b, diff, expected, actual, NULL);
        ...
        if (mpz_cmp(expected, actual) != 0) {
            mpz_clears(a, b, diff, expected, actual, NULL);
            return false;
        }
        mpz_clears(a, b, diff, expected, actual, NULL);
    }
...
void benchmark_configuration(
    ...
    printf("Sub/s: %8.2f K ", ops_per_sec_k);
...
int main(int argc, char **argv) {
    ...
    if (all_passed && !test_long_borrow_chain_case(5, 23, 24, 27, 29, 1u, false, true)) {
        all_passed = false;
    }
    if (all_passed && !test_long_borrow_chain_case(3, 1024, 1025, 1030, 1032, 1u, false, true)) {
        all_passed = false;
    }
    if (all_passed && !test_long_borrow_chain_case(1, 5000, 5001, 5000, 5001, 1u, false, true)) {
        all_passed = false;
    }
    ...
    if (all_passed) {
        ...
        for (int t = 0; t < 30; ++t) {
            ...
            const uint32_t base_len = std::max<uint32_t>(L_a, 1u);
            uint32_t L_c = 0;
            switch (t % 3) {
                case 0: L_c = std::max<uint32_t>(1u, base_len - (base_len / 3u)); break;
                case 1: L_c = base_len; break;
                default: L_c = base_len + extra_stride_dist(rng) + 7u; break;
            }
    ...
    if (all_passed && !test_long_borrow_chain_case(7, 31, 31, 31, 31, 1u, true, true)) {
        all_passed = false;
    }
    if (all_passed && !test_long_borrow_chain_case(2, 1024, 1024, 1024, 1024, 1u, true, true)) {
        all_passed = false;
    }
    if (all_passed && !test_long_borrow_chain_case(1, 5000, 5000, 5000, 5000, 1u, true, true)) {
        all_passed = false;
    }
