...
void batch_mul_ntt(uint32_t * A, uint32_t * B, uint32_t * ret, uint32_t * workspace, NTTPrecomputedTables tables, int N, int L);

// compute the workspace size needed for batch_mul_ntt
// returns the size in bytes
size_t batch_mul_ntt_workspace_size(int N, int L);
