...
bool test_shift_add(BatchMPContext *ctx) {
    struct CaseConfig {
        const char *name;
        uint32_t N;
        uint32_t L_a;
    ...
    for (const CaseConfig &cfg : cases) {
        ...
        std::vector<uint32_t> h_B((size_t)cfg.N * B.array.stride);
        std::vector<uint32_t> h_C((size_t)cfg.N * C.array.stride, 0u);
        fill_words(h_A, cfg.N, cfg.L_a, A.array.stride, 0x55AA10EFu);
        fill_words(h_B, cfg.N, cfg.L_b, B.array.stride, 0x0F1E2D3Cu);

        CUDA_CHECK(cudaMemcpy(A.array.data, h_A.data(), h_A.size() * sizeof(uint32_t), cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(B.array.data, h_B.data(), h_B.size() * sizeof(uint32_t), cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemset(C.array.data, 0, h_C.size() * sizeof(uint32_t)));

        printf("Running shift_add %-12s N=%u L_a=%u L_b=%u L_c=%u shift=%u\n",
               cfg.name, cfg.N, cfg.L_a, cfg.L_b, cfg.L_c, cfg.shift);
        CUDA_CHECK(batch_mp_shift_add(ctx, A.array, B.array, cfg.shift, C.array));
        CUDA_CHECK(cudaDeviceSynchronize());
        CUDA_CHECK(cudaMemcpy(h_C.data(), C.array.data, h_C.size() * sizeof(uint32_t), cudaMemcpyDeviceToHost));
        ...
        for (uint32_t i = 0; i < cfg.N; ++i) {
            ...
            mpz_add(expected, expected, b);
            mpz_fdiv_r_2exp(expected, expected, (mp_bitcnt_t)cfg.L_c * 32u);
            if (mpz_cmp(expected, got) != 0) {
                fprintf(stderr, "Shift-add mismatch in case %s at batch index %u\n", cfg.name, i);
                mpz_clears(a, b, expected, got, NULL);
                return false;
            }
            mpz_clears(a, b, expected, got, NULL);
        ...
        printf("  workspace: %zu -> %zu bytes\n", previous_workspace, workspace_after);
        if (cfg.L_c <= 4096 && workspace_after != previous_workspace) {
            fprintf(stderr, "Small shift-add unexpectedly changed workspace size\n");
            return false;
        }
        if (cfg.L_c > 4096 && workspace_after < previous_workspace) {
            fprintf(stderr, "Workspace size shrank after large shift-add\n");
            return false;
        }
        previous_workspace = workspace_after;
    }

    ArrayOwner BadShiftAddA{make_array(2, 4)};
    ArrayOwner BadShiftAddB{make_array(3, 5)};
    ArrayOwner BadShiftAddC{make_array(2, 6)};
    if (batch_mp_shift_add(ctx, BadShiftAddA.array, BadShiftAddB.array, 7u, BadShiftAddC.array) != cudaErrorInvalidValue) {
        fprintf(stderr, "Expected batch mismatch to fail in shift-add\n");
        return false;
    }

    ArrayOwner AliasA{make_array(2, 6)};
    ArrayOwner AliasB{make_array(2, 6)};
    if (batch_mp_shift_add(ctx, AliasA.array, AliasB.array, 5u, AliasA.array) != cudaErrorInvalidValue) {
        fprintf(stderr, "Expected C == A alias to fail in shift-add\n");
        return false;
    }
    if (batch_mp_shift_add(ctx, AliasA.array, AliasB.array, 5u, AliasB.array) != cudaErrorInvalidValue) {
        fprintf(stderr, "Expected C == B alias to fail in shift-add\n");
        return false;
    }
