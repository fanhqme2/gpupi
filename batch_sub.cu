...
__global__ void batch_sub_naive_kernel(
    const uint32_t * A,
    const uint32_t * B,
    uint32_t * C,
    uint32_t N,
...
__global__ void batch_sub_warp_kernel(
    const uint32_t * A,
    const uint32_t * B,
    uint32_t * C,
    uint32_t N,
...
__global__ void batch_sub_single_block_kernel(
    const uint32_t * A,
    const uint32_t * B,
    uint32_t * C,
    uint32_t N,
...
__global__ void batch_sub_reduce_blocks_kernel(
    const uint32_t * A,
    const uint32_t * B,
    uint32_t * C,
...
__global__ void batch_sub_combine_blocks_kernel(ushort2 * block_borrow_summary, uint32_t N, uint32_t L_c) {
    __shared__ ushort2 borrow_info[32];
...
__global__ void batch_sub_apply_blocks_kernel(
    uint32_t * C,
...
void batch_sub_simple(
    ...
    if (L_c < 32u) {
        if (L_c <= 8u) {
            ...
            batch_sub_naive_kernel<32><<<num_blocks, threads_per_block>>>(
                A, B, C, N, L_a, L_b, L_c, stride_A, stride_B, stride_C
            );
        } else {
            const uint32_t warps_per_block = 8u;
            const uint32_t num_blocks = std::min<uint32_t>((N + warps_per_block - 1u) / warps_per_block, 65536u);
            batch_sub_warp_kernel<<<num_blocks, dim3(32u, warps_per_block, 1u)>>>(
                A, B, C, N, L_a, L_b, L_c, stride_A, stride_B, stride_C
            );
        }
        return;
    ...
    if (L_c <= kChunkSize || N >= 85u || workspace == nullptr) {
        ...
        batch_sub_single_block_kernel<<<num_blocks, dim3(32u, warps_y, 1u)>>>(
            A, B, C, N, L_a, L_b, L_c, stride_A, stride_B, stride_C
        );
    } else {
        ...
        batch_sub_reduce_blocks_kernel<<<dim3(num_blocks_x, num_blocks_y, 1u), dim3(32u, 32u, 1u)>>>(
            A, B, C, block_borrow_summary, N, L_a, L_b, L_c, stride_A, stride_B, stride_C
        );
        batch_sub_combine_blocks_kernel<<<std::min<uint32_t>(N, 65536u), dim3(32u, 32u, 1u)>>>(
            block_borrow_summary, N, L_c
        );
        batch_sub_apply_blocks_kernel<<<dim3(num_blocks_x, num_blocks_y, 1u), dim3(32u, 32u, 1u)>>>(
