...
__device__ __forceinline__ uint32_t calc_len_for_add_small(uint32_t L_a, uint32_t L_c) {
    return min(L_c, max(L_a, 1u) + 1u);
}

template<int THREADS>
__global__ void batch_add_small_naive_kernel(
    const uint32_t * A,
    uint32_t B,
    uint32_t * C,
    uint32_t N,
    ...
    const uint32_t calc_len = calc_len_for_add_small(L_a, L_c);
...
__global__ void batch_add_small_warp_kernel(
    const uint32_t * A,
    uint32_t B,
    uint32_t * C,
    uint32_t N,
    ...
    for (uint32_t idx = warp_global; idx < N; idx += warp_stride) {
        ...
        if (lane < calc_len) {
            const uint32_t a = (lane < L_a) ? a_row[lane] : 0u;
            const uint32_t addend = (lane == 0u) ? B : 0u;
            word = a + addend;
...
__global__ void batch_add_small_single_block_kernel(
    const uint32_t * A,
    uint32_t B,
    uint32_t * C,
    bool in_place,
    ...
    for (uint32_t idx = blockIdx.x; idx < N; idx += gridDim.x) {
        ...
        for (uint32_t i0 = 0; i0 < L_c; i0 += threads_per_block * 2u) {
            ...
            const uint32_t c0_value = (i == 0u) ? B : 0u;
            ushort2 carry;
            ...
            if (in_place && block_carry_shared == 0) {
                break;
            }
        }
    }
...
__global__ void batch_add_small_reduce_blocks_kernel(
    const uint32_t * A,
    uint32_t B,
    uint32_t * C,
    ...
    for (uint32_t number_idx = blockIdx.y; number_idx < N; number_idx += gridDim.y) {
        ...
        for (uint32_t chunk_start = blockIdx.x * kChunkSize, chunk_idx = blockIdx.x;
            ...
            for (uint32_t i0 = 0; i0 < chunk_len; i0 += threads_per_block * 2u) {
                ...
                const uint32_t c0_value = (g0 == 0u && i < chunk_len) ? B : 0u;
                ushort2 carry;
...
__global__ void batch_add_small_combine_blocks_kernel(ushort2 * block_carry_summary, uint32_t N, uint32_t L_c) {
    __shared__ ushort2 carry_info[32];
...
__global__ void batch_add_small_apply_blocks_kernel(
    uint32_t * C,
...
void batch_add_small(
    ...
    const bool in_place = (A == C);
    if (in_place) {
        assert(L_a == L_c);
        assert(stride_A == stride_C);
    }
    ...
    if (L_c < 32u) {
        if (L_c <= 8u) {
            ...
            batch_add_small_naive_kernel<32><<<num_blocks, threads_per_block>>>(
            ...
            batch_add_small_warp_kernel<<<num_blocks, dim3(32u, warps_per_block, 1u)>>>(
                A, B, C, N, L_a, L_c, stride_A, stride_C
            );
        }
        return;
    ...
    if (L_c <= kChunkSize || N >= 85u || workspace == nullptr) {
        ...
        batch_add_small_single_block_kernel<<<num_blocks, dim3(32u, warps_y, 1u)>>>(
            A, B, C, in_place, N, L_a, L_c, stride_A, stride_C
        );
    } else {
        ...
        batch_add_small_reduce_blocks_kernel<<<dim3(num_blocks_x, num_blocks_y, 1u), dim3(32u, 32u, 1u)>>>(
            A, B, C, block_carry_summary, N, L_a, L_c, stride_A, stride_C
        );
        batch_add_small_combine_blocks_kernel<<<std::min<uint32_t>(N, 65536u), dim3(32u, 32u, 1u)>>>(
            block_carry_summary, N, L_c
        );
        batch_add_small_apply_blocks_kernel<<<dim3(num_blocks_x, num_blocks_y, 1u), dim3(32u, 32u, 1u)>>>(
