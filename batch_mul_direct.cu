...
__global__ void batch_mul_direct_kernel4(uint32_t * A, uint32_t * B, uint32_t * ret, int N, int L){
    ...
    for (int idx = blockIdx.x; idx < N; idx += gridDim.x){
        ...
        __syncthreads();
        for (int i0 = 0; i0 < L; i0 += BLOCK_SIZE){
            uint32_t a_value;
            uint32_t r_value;
            a_value = (i0 + threadIdx.x < L) ? a[i0 + threadIdx.x] : 0;
            r_value = (i0 + threadIdx.x < L * 2) ? r[i0 + threadIdx.x] : 0;
            uint32_t c_value = 0;
            for (int j = 0; j < L; j ++){
                uint32_t b_value = b[j];
                uint64_t mul = (uint64_t)a_value * (uint64_t)b_value + r_value + c_value;
                r_value = (uint32_t)mul;
                c_value = mul >> 32;
                if (threadIdx.x == 0){
                    r[i0 + j] = r_value;
                }
                r_value = __shfl_down_sync(warp_mask, r_value, 1, BLOCK_SIZE);
                if (threadIdx.x == BLOCK_SIZE - 1){
                    r_value = r[i0 + j + BLOCK_SIZE];
                }
            }
            add_cc(0, 0);
            for (int j = 0; j < BLOCK_SIZE; j ++){
                if (threadIdx.x == 0){
                    r_value = addc_cc(r_value, c_value);
                    r[i0 + j + L] = r_value;
                }
                r_value = __shfl_down_sync(warp_mask, r_value, 1, BLOCK_SIZE);
                c_value = __shfl_down_sync(warp_mask, c_value, 1, BLOCK_SIZE);
            }
        }
        __syncthreads();
        for (int i = threadIdx.x; i < L * 2; i += BLOCK_SIZE){
...
__global__ void batch_mul_direct_kernel5(uint32_t * A, uint32_t * B, uint32_t * ret, int N, int L){
    ...
    for (int idx = blockIdx.x; idx < N; idx += gridDim.x){
        ...
        for (int i0 = 0; i0 < L; i0 += BLOCK_SIZE * 2){
            ...
            for (int j = 0; j < BLOCK_SIZE; j ++){
                if (threadIdx.x == 0){
                    r0_value = addc_cc(r0_value, c0_value);
                    r1_value = addc_cc(r1_value, c1_value);
                    r[i0 + j * 2 + L2] = r0_value;
                    r[i0 + j * 2 + 1 + L2] = r1_value;
                }
                r0_value = __shfl_down_sync(warp_mask, r0_value, 1, BLOCK_SIZE);
                r1_value = __shfl_down_sync(warp_mask, r1_value, 1, BLOCK_SIZE);
                c0_value = __shfl_down_sync(warp_mask, c0_value, 1, BLOCK_SIZE);
                c1_value = __shfl_down_sync(warp_mask, c1_value, 1, BLOCK_SIZE);
            }*/
        }
        __syncthreads();
        for (int i = threadIdx.x; i < L * 2; i += BLOCK_SIZE){
...
template<int BLOCK_SIZE>
__global__ void batch_mul_direct_kernel6(uint32_t * A, uint32_t * B, uint32_t * ret, int N, int L){
    __shared__ uint32_t a[BATCH_MUL_DIRECT_L_MAX];
    __shared__ uint32_t b[BATCH_MUL_DIRECT_L_MAX];
    __shared__ uint32_t r[BATCH_MUL_DIRECT_L_MAX * 2];
    const unsigned int warp_mask = (1ull << BLOCK_SIZE) - 1;
    for (int i0 = 0; i0 < L; i0 += BLOCK_SIZE * 4){
        a[i0 + threadIdx.x * 4] = 0;
        a[i0 + threadIdx.x * 4 + 1] = 0;
        a[i0 + threadIdx.x * 4 + 2] = 0;
        a[i0 + threadIdx.x * 4 + 3] = 0;
        b[i0 + threadIdx.x * 4] = 0;
        b[i0 + threadIdx.x * 4 + 1] = 0;
        b[i0 + threadIdx.x * 4 + 2] = 0;
        b[i0 + threadIdx.x * 4 + 3] = 0;
    }
    for (int idx = blockIdx.x; idx < N; idx += gridDim.x){
        for (int i = threadIdx.x; i < L; i += BLOCK_SIZE){
            a[i] = A[idx * L + i];
        ...
        for (int i0 = 0; i0 < L * 2; i0 += BLOCK_SIZE * 4){
            r[i0 + threadIdx.x * 4] = 0;
            r[i0 + threadIdx.x * 4 + 1] = 0;
            r[i0 + threadIdx.x * 4 + 2] = 0;
            r[i0 + threadIdx.x * 4 + 3] = 0;
        }
        __syncthreads();
        for (int i0 = 0; i0 < L; i0 += BLOCK_SIZE * 4){
            uint32_t a0_value, a1_value, a2_value, a3_value;
            uint32_t r0_value, r1_value, r2_value, r3_value;
            a0_value = a[i0 + threadIdx.x * 4];
            a1_value = a[i0 + threadIdx.x * 4 + 1];
            a2_value = a[i0 + threadIdx.x * 4 + 2];
            a3_value = a[i0 + threadIdx.x * 4 + 3];
            r0_value = r[i0 + threadIdx.x * 4];
            r1_value = r[i0 + threadIdx.x * 4 + 1];
            r2_value = r[i0 + threadIdx.x * 4 + 2];
            r3_value = r[i0 + threadIdx.x * 4 + 3];
            uint32_t c0_value = 0, c1_value = 0, c2_value = 0, c3_value = 0;
            for (int j = 0; j < L; j += 4){
                uint32_t b0_value = b[j];
                uint32_t b1_value = b[j + 1];
                uint32_t b2_value = b[j + 2];
                uint32_t b3_value = b[j + 3];
                ...
                uint64_t mul02 = (uint64_t)a0_value * (uint64_t)b2_value + r2_value + c2_value;
                uint32_t mid22 = (uint32_t)mul02;
                uint32_t mid30 = mul02 >> 32;

                uint64_t mul11 = (uint64_t)a1_value * (uint64_t)b1_value + mid20 + mid21;
                uint32_t mid23 = (uint32_t)mul11;
                uint32_t mid31 = mul11 >> 32;

                uint64_t mul20 = (uint64_t)a2_value * (uint64_t)b0_value + mid22 + mid23;
                r2_value = (uint32_t)mul20;
                uint32_t mid32 = mul20 >> 32;

                // r3, c3, mid30, mid31, mid32

                uint64_t mul03 = (uint64_t)a0_value * (uint64_t)b3_value + r3_value + c3_value;
                uint32_t mid33 = (uint32_t)mul03;
                uint32_t mid40 = mul03 >> 32;

                uint64_t mul12 = (uint64_t)a1_value * (uint64_t)b2_value + mid30 + mid31;
                uint32_t mid34 = (uint32_t)mul12;
                uint32_t mid41 = mul12 >> 32;

                uint64_t mul21 = (uint64_t)a2_value * (uint64_t)b1_value + mid32 + mid33;
                uint32_t mid35 = (uint32_t)mul21;
                uint32_t mid42 = mul21 >> 32;

                uint64_t mul30 = (uint64_t)a3_value * (uint64_t)b0_value + mid34 + mid35;
                r3_value = (uint32_t)mul30;
                uint32_t mid43 = mul30 >> 32;

                // mid40, mid41, mid42, mid43

                uint64_t mul13 = (uint64_t)a1_value * (uint64_t)b3_value + mid40 + mid41;
                uint32_t mid44 = (uint32_t)mul13;
                uint32_t mid50 = mul13 >> 32;

                uint64_t mul22 = (uint64_t)a2_value * (uint64_t)b2_value + mid42 + mid43;
                uint32_t mid45 = (uint32_t)mul22;
                uint32_t mid51 = mul22 >> 32;

                uint64_t mul31 = (uint64_t)a3_value * (uint64_t)b1_value + mid44 + mid45;
                c0_value = (uint32_t)mul31;
                uint32_t mid52 = mul31 >> 32;

                // mid50, mid51, mid52
                uint64_t mul23 = (uint64_t)a2_value * (uint64_t)b3_value + mid50 + mid51;
                uint32_t mid53 = (uint32_t)mul23;
                uint32_t mid60 = mul23 >> 32;

                uint64_t mul32 = (uint64_t)a3_value * (uint64_t)b2_value + mid52 + mid53;
                c1_value = (uint32_t)mul32;
                uint32_t mid61 = mul32 >> 32;

                // mid60, mid61
                uint64_t mul33 = (uint64_t)a3_value * (uint64_t)b3_value + mid60 + mid61;
                c2_value = (uint32_t)mul33;
                c3_value = mul33 >> 32;
                ...
                r0_value = __shfl_down_sync(warp_mask, r0_value, 1, BLOCK_SIZE);
                r1_value = __shfl_down_sync(warp_mask, r1_value, 1, BLOCK_SIZE);
                r2_value = __shfl_down_sync(warp_mask, r2_value, 1, BLOCK_SIZE);
                r3_value = __shfl_down_sync(warp_mask, r3_value, 1, BLOCK_SIZE);
                if (threadIdx.x == BLOCK_SIZE - 1){
                    r0_value = r[i0 + j + BLOCK_SIZE * 4];
                    r1_value = r[i0 + j + BLOCK_SIZE * 4 + 1];
                    r2_value = r[i0 + j + BLOCK_SIZE * 4 + 2];
                    r3_value = r[i0 + j + BLOCK_SIZE * 4 + 3];
                }
            }
            int L4 = (L + 3) & -4;
            add_cc(0, 0);
            for (int j = 0; j < BLOCK_SIZE; j ++){
                if (threadIdx.x == 0){
                    r0_value = addc_cc(r0_value, c0_value);
                    r1_value = addc_cc(r1_value, c1_value);
                    r2_value = addc_cc(r2_value, c2_value);
                    r3_value = addc_cc(r3_value, c3_value);
                    r[i0 + j * 4 + L4] = r0_value;
                    r[i0 + j * 4 + 1 + L4] = r1_value;
                    r[i0 + j * 4 + 2 + L4] = r2_value;
                    r[i0 + j * 4 + 3 + L4] = r3_value;
                }
                r0_value = __shfl_down_sync(warp_mask, r0_value, 1, BLOCK_SIZE);
                r1_value = __shfl_down_sync(warp_mask, r1_value, 1, BLOCK_SIZE);
                r2_value = __shfl_down_sync(warp_mask, r2_value, 1, BLOCK_SIZE);
                ...
                c0_value = __shfl_down_sync(warp_mask, c0_value, 1, BLOCK_SIZE);
                c1_value = __shfl_down_sync(warp_mask, c1_value, 1, BLOCK_SIZE);
                c2_value = __shfl_down_sync(warp_mask, c2_value, 1, BLOCK_SIZE);
                c3_value = __shfl_down_sync(warp_mask, c3_value, 1, BLOCK_SIZE);
            }
        }
        __syncthreads();
        for (int i = threadIdx.x; i < L * 2; i += BLOCK_SIZE){
