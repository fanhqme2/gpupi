...
__device__ __forceinline__ void batch_sub_64_all_warp(
    uint32_t & r0_value,
    uint32_t & r1_value,
    uint32_t & c0_value,
    uint32_t & c1_value,
...
__global__ void batch_sub_small_naive_kernel(
    const uint32_t * A,
    uint32_t B,
    uint32_t * C,
    uint32_t N,
...
__global__ void batch_sub_small_warp_kernel(
    const uint32_t * A,
    uint32_t B,
    uint32_t * C,
    uint32_t N,
    ...
    for (uint32_t idx = warp_global; idx < N; idx += warp_stride) {
        ...
        if (lane < L_c) {
            ...
            const uint32_t subtrahend = (lane == 0u) ? B : 0u;
            word = a - subtrahend;
            borrow_state = (a < subtrahend) ? 2u : (word == 0u ? 1u : 0u);
        }
...
__global__ void batch_sub_small_single_block_kernel(
    const uint32_t * A,
    uint32_t B,
    uint32_t * C,
    bool in_place,
    ...
    for (uint32_t idx = blockIdx.x; idx < N; idx += gridDim.x) {
        ...
        for (uint32_t i0 = 0; i0 < L_c; i0 += threads_per_block * 2u) {
            ...
            if (in_place && block_borrow_shared == 0) {
                break;
            }
        }
    }
...
__global__ void batch_sub_small_reduce_blocks_kernel(
    const uint32_t * A,
    uint32_t B,
    uint32_t * C,
...
__global__ void batch_sub_small_combine_blocks_kernel(ushort2 * block_borrow_summary, uint32_t N, uint32_t L_c) {
    __shared__ ushort2 borrow_info[32];
...
__global__ void batch_sub_small_apply_blocks_kernel(
    uint32_t * C,
    ...
    for (uint32_t number_idx = blockIdx.y; number_idx < N; number_idx += gridDim.y) {
        ...
        for (uint32_t chunk_start = blockIdx.x * kChunkSize, chunk_idx = blockIdx.x;
            ...
            for (uint32_t i0 = 0; i0 < chunk_len; i0 += blockDim.x * blockDim.y * 2u) {
                ...
                batch_sub_64_all_warp(r0_value, r1_value, c0_value, c1_value, borrow_info);
...
void batch_sub_small(
    ...
    if (L_c < 32u) {
        if (L_c <= 8u) {
            ...
            batch_sub_small_naive_kernel<32><<<num_blocks, threads_per_block>>>(
            ...
            batch_sub_small_warp_kernel<<<num_blocks, dim3(32u, warps_per_block, 1u)>>>(
                A, B, C, N, L_a, L_c, stride_A, stride_C
            );
        }
        return;
    ...
    if (L_c <= kChunkSize || N >= 85u || workspace == nullptr) {
        ...
        batch_sub_small_single_block_kernel<<<num_blocks, dim3(32u, warps_y, 1u)>>>(
        ...
        batch_sub_small_reduce_blocks_kernel<<<dim3(num_blocks_x, num_blocks_y, 1u), dim3(32u, 32u, 1u)>>>(
            A, B, C, block_borrow_summary, N, L_a, L_c, stride_A, stride_C
        );
        batch_sub_small_combine_blocks_kernel<<<std::min<uint32_t>(N, 65536u), dim3(32u, 32u, 1u)>>>(
            block_borrow_summary, N, L_c
        );
        batch_sub_small_apply_blocks_kernel<<<dim3(num_blocks_x, num_blocks_y, 1u), dim3(32u, 32u, 1u)>>>(
