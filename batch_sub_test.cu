...
void print_limb_window(
    ...
    for (uint32_t i = 0; i < L_c; ++i) {
        ...
        expected[i] = (uint32_t)diff;
        borrow = ((uint64_t)b + borrow > a) ? 1u : 0u;
    }
    ...
    for (size_t s = 0; s < sample_indices.size(); ++s) {
        ...
        if (mpz_cmp(expected, actual) != 0) {
            ...
            for (; mismatch_idx < L_c; ++mismatch_idx) {
                ...
                const uint64_t diff_word = av - bv - borrow;
                if ((uint32_t)diff_word != c_row[mismatch_idx]) {
                    break;
                }
                borrow = ((uint64_t)bv + borrow > av) ? 1u : 0u;
            }
            if (mismatch_idx == L_c && L_c > 0) {
                mismatch_idx = L_c - 1;
            }
            print_limb_window(a_row, b_row, c_row, L_a, L_b, L_c, mismatch_idx);
...
void benchmark_configuration(
    ...
    const double subs_per_sec_k = ((double)N / (avg_ms * 1e-3)) / 1e3;
    ...
    printf("Sub/s: %8.2f K ", subs_per_sec_k);
...
int main(int argc, char **argv) {
    ...
    if (all_passed) {
        ...
        for (int t = 0; t < 30; ++t) {
            ...
            uint32_t L_c = 0;
            switch (t % 3) {
                case 0: L_c = std::max<uint32_t>(L_a, L_b); break;
                case 1: L_c = std::max<uint32_t>(L_a, L_b) + extra_stride_dist(rng) + 1u; break;
                default: L_c = (std::max<uint32_t>(L_a, L_b) == 0) ? 0u :
                                   std::max<uint32_t>(1u, std::max<uint32_t>(L_a, L_b) / 2u); break;
            }
